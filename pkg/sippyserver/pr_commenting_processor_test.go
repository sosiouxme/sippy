			...
			fetchJobRun: func(dbc *db.DB, jobRunID int64, unknownTests bool, logger *logrus.Entry) (*models.ProwJobRun, int, error) {
				pjr := models.ProwJobRun{
					Tests: []models.ProwJobRunTest{
						{Test: models.Test{Name: "test1"}, Status: int(spv1.TestStatusSuccess)},
						{Test: models.Test{Name: "test2"}, Status: int(spv1.TestStatusFailure)},
						{Test: models.Test{Name: "test3"}, Status: int(spv1.TestStatusFlake)},
					},
				}
